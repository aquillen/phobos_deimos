...
// quadrupole of planet   ** this could be rotating!
double J2_plus = 0.0; // is greater than zero for an oblate planet
double R_plus = 1.0;
double theta_plus = 0.0;
double phi_plus = 0.0;
double Mars_omega = 0.0; // Mars rotation rate
...
void additional_forces(struct reb_simulation* r){
   ...
   // quadC22S22(r,C22,S22,R_plus,r->N-npert);
   // quadJ2pole(r,J2_plus,R_plus,theta_plus,phi_plus,r->N-npert); // quad force
   double C20 = -J2_plus;
   quadforce(r, C20, C22_0, 0.0, R_plus, Mars_omega,r->N-npert);
}
...
// things to set!  can be read in with parameter file
	...
	meananom[ip] =0.0;    // initial 
        ...
        fgets(line,300,fpi);  sscanf(line,"%lf %lf %lf %lf %lf"
              ,&J2_plus,&R_plus,&theta_plus,&C22_0,&Mars_omega); // for an oblate planet
...
// fprintf(fpr,"adot %.3e\n",adot);
   ...
   if (J2_plus != 0.0){
     ...
     fprintf(fpr,"theta_plus %.3f\n", theta_plus);
     fprintf(fpr,"Mars_omega %.3f\n", Mars_omega);
     fprintf(fpr,"C22_0 %.3e\n", C22_0);
   }
